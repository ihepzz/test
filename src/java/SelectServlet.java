...
public class SelectServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("select nameis :" + name);

        if (name.equals("直线加速器")) {
            ...
            request.setAttribute("systemteamList", systemList);
        } else if(name.equals("增强器")){
            TeamJpaController teamJpaController = new TeamJpaController();
            List<Team> teamList = teamJpaController.findTeamEntities();
            request.setAttribute("systemteamList", teamList);
        }
         request.getRequestDispatcher("datajsp.jsp").forward(request, response);
