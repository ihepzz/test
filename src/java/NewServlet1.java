...
public class NewServlet1 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String name = (String) request.getParameter("child");
        System.out.println("nameis :" + name);

        DataJpaController dataJpaController = new DataJpaController();
        List<Data> dataList = dataJpaController.findDataBySystem(name);
        request.setAttribute("dataList", dataList);

        request.getRequestDispatcher("datajsp1.jsp").forward(request, response);
//response.sendRedirect("dataresult.jsp");
    }
